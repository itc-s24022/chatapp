...
export const saveUserInfo = async (userId, userData) => {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            // 個人用チャンネルを作成
            await createPersonalChannel(userId, userData.displayName || '匿名');
        }
    } catch (error) {
...
// 個人用チャンネル作成関数
export const createPersonalChannel = async (userId, userName) => {
    try {
        // 既存の個人用チャンネルがあるか確認
        const channelsRef = collection(db, 'channels');
        const q = query(
        ...
        if (querySnapshot.empty) {
            ...
            const channelData = {
                ...
                participants: [userId],
                createdAt: serverTimestamp(),
                description: '個人用のメモやアイデアを保存するスペース'
            };
            await addDoc(channelsRef, channelData);
            console.log('個人用チャンネル作成完了');
        }
    } catch (error) {
        console.error('個人用チャンネル作成エラー:', error);
        throw error;
    }
};

// グローバルチャンネルの取得
export const getGlobalChannels = (callback) => {
    const q = query(
        collection(db, 'channels'),
        where('type', '==', 'global'),
        orderBy('name', 'asc')
...
export const getAllUserChannels = (userId, callback) => {
    // 個人用チャンネル
    ...
    const unsubscribeServer = onSnapshot(serverQuery, (snapshot) => {
        updateChannelsList();
    });
    ...
    const updateChannelsList = async () => {
        try {
            ...
            const personalChannels = personalSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));

            const dmChannels = dmSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));

            const globalChannels = globalSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));

            const serverChannels = serverSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));
            ...
            // 全チャンネルを結合してソート
            const allChannels = [
                ...personalChannels.map(ch => ({...ch, category: 'personal'})),
                ...dmChannels.map(ch => ({...ch, category: 'dm'})),
                ...globalChannels.map(ch => ({...ch, category: 'global'})),
                ...serverChannels.map(ch => ({...ch, category: 'server'}))
            ];
...
// グローバルチャンネルへのメッセージ送信
export const sendGlobalMessage = async (channelId, userId, userName, content) => {
    try {
        const messageData = {
            channelId,
            userId,
        ...
        console.log('グローバルメッセージ送信完了');
    } catch (error) {
        console.error('グローバルメッセージ送信エラー:', error);
        throw error;
    }
};
...
export const updateUserTags = async (userId, tags) => {
    try {
        ...
        console.log('ユーザータグ更新完了');
    } catch (error) {
        console.error('ユーザータグ更新エラー:', error);
        throw error;
    }
};

export const searchUsersByTag = async (tag, excludeUserIds = []) => {
    try {
        ...
        const users = snapshot.docs
            .map(doc => ({ id: doc.id, ...doc.data() }))
            .filter(user => !excludeUserIds.includes(user.uid || user.id));
        return users;
    } catch (error) {
        console.error('タグによるユーザー検索エラー:', error);
        return [];
    }
};

export const inviteUsersByTag = async (serverId, tag, inviterName, excludeUserIds = []) => {
    try {
        // タグでユーザーを検索
        const users = await searchUsersByTag(tag, excludeUserIds);
        if (users.length === 0) {
            ...
            try {
                ...
                if (memberSnapshot.empty) {
                    ...
                    if (inviteSnapshot.empty) {
                        ...
                        await addDoc(collection(db, 'serverInvites'), inviteData);
                ...
                console.error(`ユーザー ${user.displayName || user.uid} への招待エラー:`, error);
                failedInvites.push({
                    userId: user.uid || user.id,
                    userName: user.displayName || '匿名',
                    reason: 'エラーが発生しました'
        ...
        console.error('タグによる一括招待エラー:', error);
        throw error;
    }
};
    ...
    try {
        ...
        snapshot.docs.forEach(doc => {
            ...
            if (userData.tags && Array.isArray(userData.tags)) {
                userData.tags.forEach(tag => allTags.add(tag));
            }
        });
...
export const getUsersByTags = (tags, callback, errorCallback) => {
    try {
        ...
        console.error('タグによるユーザー取得エラー:', error);
        if (errorCallback) errorCallback(error);
    }
};

export const addUserTag = async (userId, newTag) => {
    try {
        const userQuery = query(
            collection(db, 'users'),
            where('uid', '==', userId)
        ...
        console.log('ユーザータグ追加完了');
    } catch (error) {
        console.error('ユーザータグ追加エラー:', error);
        throw error;
    }
};
...
export const removeUserTag = async (userId, tagToRemove) => {
    try {
        ...
        console.log('ユーザータグ削除完了');
    } catch (error) {
        console.error('ユーザータグ削除エラー:', error);
        throw error;
    }
};