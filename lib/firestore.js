...
// getUserServers関数の修正例
export async function getUserServers(userId, callback, errorCallback) {
    if (!userId) {
        if (errorCallback) errorCallback(new Error('ユーザーIDが提供されていません'));
        return () => {};
    }
...
// getUserDMs関数の修正例
export async function getUserDMs(userId, callback) {
    if (!userId) return () => {};
